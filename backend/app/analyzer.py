...
def analyze(files: list[SourceFile]) -> tuple[list[Node], list[Edge], str, list[str]]:
    normalized = [SourceFile(path=_path(f.path), content=f.content) for f in files if f.path]
    ...
    for file in normalized:
        ...
        nodes.append(Node(id=file.path, label=PurePosixPath(file.path).name, language=language, summary=purpose + ".", symbols=symbols))
    ...
    overview = f"This codebase has {len(nodes)} files, {total_symbols} discovered top-level symbols, and {len(edges)} internal dependency links."
    ...
    return nodes, edges, overview, insights
